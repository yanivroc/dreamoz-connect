...
- **Payments**: plans use **live (production) Square** settings. The product cart/checkout stays on the **sandbox** settings exactly as it is today.
...
## 4 new settings to add in Vercel (and Lovable)
...
Until these are set, the Plan tab shows "Payments not configured yet" and doesn't take any cards.
...
- `src/lib/billing.functions.ts`: `getBillingConfig` (reads SQUARE_PROD_*), `purchasePlan({plan, sourceId})`. This checks the session, takes the amount from the server-side config only, charges through Square production with an idempotency key, records the payment, and extends `plan_expires_at = max(now, current) + days`.
...
- `src/components/PlanPanel.tsx`: loads the Square Web Payments SDK from the production URL, renders the card form, shows toasts on success or failure, then refreshes the user.
